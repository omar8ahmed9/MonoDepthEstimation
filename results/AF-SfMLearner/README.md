...
For all reported SCARED benchmark numbers in this workspace, we use the EndoVis split resolved by `AF-SfMLearner/evaluate_depth.py` with evaluation on the `endovis` split.
...
```bash
conda run -n afsfm python AF-SfMLearner/evaluate_depth.py \
  --data_path /l/users/saif.alkindi/datasets/SCARED_DARES \
  --load_weights_folder /l/users/saif.alkindi/projects/ai7102_project/AF-SfMLearner/pretrained/Model_trained_end_to_end \
  --eval_mono \
  --num_workers 0
